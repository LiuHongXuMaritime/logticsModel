...
# 工厂到DC候选点的距离（Loj）
Loj = arr[2, :]
Lport_j = np.min(arr[[10, 11], :], axis=0)
# DC候选点到每个客户的距离矩阵（Lji）
Lji = arr
...
# 船数
n = model.addVar(vtype=GRB.INTEGER, lb=0, name="Ship_number")
# ===================== 4. 改进后的目标函数 =====================
# 成本项1：工厂→DC的干线运输成本
cost_trans_factory_dc = gp.quicksum(4 * qS[j] + (4 + 0.03 * Lport_j[j])*qS[j] for j in range(n_dc_candidates))
...
# 成本项4：DC库存持有成本（按周换算）
cost_inventory = gp.quicksum(qS[j] * 600 * 0.2 * (1 / 52) * (6 / 7) for j in range(n_dc_candidates))
...
model.addConstr(
    n*3000 >= gp.quicksum(qS[j] for j in range(n_dc_candidates)),
...
df.to_excel(f"DC选址结果4_价格{model.objVal:.2f}.xlsx")

dcnparr = df["是否建立DC(y_j)"].to_numpy()
DClst = np.where(dcnparr == 1)[0]

plot_result(DClst, [qS[j].X for j in range(n_dc_candidates)],
            [[qji[j, i].X for i in range(n_customers)] for j in range(n_dc_candidates)],title=f"物流网络拓扑图4_价格{model.objVal:.2f}")
