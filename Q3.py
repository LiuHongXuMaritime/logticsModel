...
# 火车车皮箱数
mj = model.addVars(n_dc_candidates, vtype=GRB.INTEGER, lb=0, name="Train_Box")
# ===================== 4. 改进后的目标函数 =====================
# 成本项1：工厂→DC的干线运输成本
cost_trans_factory_dc = gp.quicksum(50 * mj[j] + 0.45 * Loj[j] * mj[j] for j in range(n_dc_candidates))
...
# 成本项4：DC库存持有成本（按周换算）
cost_inventory = gp.quicksum(qS[j] * 600 * 0.2 * (1 / 52) * 3 for j in range(n_dc_candidates))
...
df["车车皮数(m_j)"] = [mj[j].X for j in range(n_dc_candidates)]
for i in range(n_customers):
    df[f"DC到客户{i}配送量(qji_j_{i})"] = [qji[j, i].X for j in range(n_dc_candidates)]
df.to_excel(f"DC选址结果3_价格{model.objVal:.2f}.xlsx")

dcnparr = df["是否建立DC(y_j)"].to_numpy()
DClst = np.where(dcnparr == 1)[0]

plot_result(DClst, [qS[j].X for j in range(n_dc_candidates)],
            [[qji[j, i].X for i in range(n_customers)] for j in range(n_dc_candidates)],title=f"物流网络拓扑图3_价格{model.objVal:.2f}")
