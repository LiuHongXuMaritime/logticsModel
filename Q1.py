...
import numpy as np
import pandas as pd
from plot_function import plot_result
...
# ===================== 4. 改进后的目标函数 =====================
# 成本项1：工厂→DC的干线运输成本
cost_trans_factory_dc = gp.quicksum((4 + 0.03 * Loj[j]) * qS[j] for j in range(n_dc_candidates))
...
df.to_excel(f"DC选址结果1_价格{model.objVal:.2f}.xlsx")
dcnparr = df["是否建立DC(y_j)"].to_numpy()
DClst = np.where(dcnparr == 1)[0]

plot_result(DClst, [qS[j].X for j in range(n_dc_candidates)],
            [[qji[j, i].X for i in range(n_customers)] for j in range(n_dc_candidates)],title=f"物流网络拓扑图1_价格{model.objVal:.2f}")
